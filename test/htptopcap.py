    ...
    def run(self):
        ...
        for l in f.readlines():
            ...
            if l.rstrip() == "<<<" or l.rstrip() == ">>>":
                ...
                if state == 1:
                    conn.send(sending.rstrip())
                    print "server sent", len(sending.rstrip())
                elif state == 2:
                    data = conn.recv(len(receiving.rstrip()))
...
class ClientThread(Thread):
    ...
    def run(self):
        ...
        for l in f.readlines():
            if l.rstrip() == "<<<" or l.rstrip() == ">>>":
                if state == 1:
                    s.send(sending.rstrip())
                    print "client sent", len(sending.rstrip())
                elif state == 2:
                    data = s.recv(len(receiving.rstrip()))
